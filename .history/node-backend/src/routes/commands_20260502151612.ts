...
router.post('/process', asyncHandler(async (req: Request, res: Response) => {
  const { token, userId, audioBuffer } = req.body;
  ...
  try {
    ...
    speechFormData.append('file', bufferData, 'audio.wav');
    speechFormData.append('language', 'en');

    const speechResponse = await axios.post(
      `${PYTHON_API_URL}/api/speech/transcribe`,
      speechFormData,
      {
        headers: {
          ...speechFormData.getHeaders(),
          'Authorization': `Bearer ${token}`
        }
      }
    );

    const { text: command, confidence } = speechResponse.data;
