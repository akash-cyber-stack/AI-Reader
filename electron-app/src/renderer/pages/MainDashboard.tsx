...
export const MainDashboard: React.FC<MainDashboardProps> = ({
  ...
  const processCommand = async () => {
    try {
      ...
      if (!commandText) {
        setStatus('ERROR');
        setResponseText('Could not hear command');
        speak('I could not hear your command. Please try again.');
        return;
      }
      ...
      if (!response.ok) {
        ...
        speak(data.error || data.message || 'Access denied');
        return;
      }
      ...
      setStatus('ERROR');
      setResponseText(error.message || 'Error processing command');
